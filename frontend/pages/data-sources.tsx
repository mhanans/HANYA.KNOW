    ...
    <div className="page-container">
      ...
      <div className="card">
        ...
        <div className="chat-box">
          {messages.map((m, i) => (
            <p key={i} className={m.role === 'user' ? 'user-msg' : 'bot-msg'}>{m.content}</p>
          ))}
        </div>
        <div className="form-grid">
          <label>Question</label>
          <input className="form-input" value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key==='Enter' ? ask() : null} />
        </div>
        <div className="actions">
          <button className="btn btn-primary" onClick={ask} disabled={asking}>{asking ? 'Asking...' : 'Ask'}</button>
        </div>
      </div>
    </div>
  );
